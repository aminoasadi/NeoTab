...
export function SearchAndBookmarks() {
  ...
  const bookmarks = [
    { id: 1, title: 'Neo Core (داشبورد اصلی)', icon: LinkIcon, color: 'text-indigo-500', bg: 'bg-indigo-50 dark:bg-indigo-500/10' },
    ...
    <div className="w-full max-w-3xl mx-auto flex flex-col items-center gap-6 mt-16 mb-10 relative z-40">
      ...
      <div className="w-full glass-panel rounded-3xl p-4 flex flex-wrap justify-center gap-3">
        {bookmarks.map((bookmark) => (
          <div key={bookmark.id} className="flex items-center gap-2 px-4 py-2.5 bg-white/60 dark:bg-slate-800/60 rounded-2xl border border-slate-200/50 dark:border-slate-700/50 hover:shadow-md hover:-translate-y-0.5 transition-all cursor-pointer group">
