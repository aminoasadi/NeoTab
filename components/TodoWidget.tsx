...
import { ListTodo, Plus, MoreVertical, Calendar, Check, X } from 'lucide-react';
...
export function TodoWidget() {
  ...
  const [newTaskDue, setNewTaskDue] = useState('امروز');
    ...
    <>
      <div className="glass-panel rounded-3xl p-5 flex flex-col gap-4 h-[300px]">
        <div className="flex items-center justify-between">
          ...
          <button onClick={() => setIsModalOpen(true)} className="p-1.5 text-slate-500 hover:text-indigo-600 bg-white/50 dark:bg-slate-800/50 rounded-xl shadow-sm border border-slate-200/50 dark:border-slate-700/50 transition-colors">
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/50 backdrop-blur-sm">
          <div className="bg-white dark:bg-slate-800 p-6 rounded-3xl w-full max-w-sm shadow-2xl relative">
            <button
            ...
            <div className="flex flex-col gap-4">
              <div>
                <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">عنوان کار</label>
                <input
                  type="text"
                  value={newTaskText}
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">اولویت</label>
                  <select
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">زمان انجام</label>
                  <DatePicker
                    value={newTaskDue}
                    onChange={(date: any) => setNewTaskDue(date?.format() || 'امروز')}
                    calendar={persian}
                    locale={persian_fa}
                    calendarPosition="bottom-right"
                    inputClass="w-full px-3 py-2 rounded-xl border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 text-slate-800 dark:text-slate-100 placeholder:text-slate-400"
                    placeholder="مثلا: فردا"
                  />
                </div>
              </div>
